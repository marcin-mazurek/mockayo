...
test.serial(
  "remembers the scenario set with POST /__admin/set-scenario",
  async t => {
    ...
    const endpointResponseBody = await endpointResponse.text();

    t.is(endpointResponse.status, 500);
    t.is(endpointResponseBody, "Something went wrong!");
  }
);
